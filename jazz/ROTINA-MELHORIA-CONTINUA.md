...
a0. Registrar cada busca da LP: uma linha JSON por chamada via console.log na função `netlify/functions/buscar.mjs` (parâmetros, portais consultados, total por portal, motivo, duração; IP truncado; NUNCA nome/telefone) — vira log de função no painel do Netlify sem depender do Supabase. Manter `node teste-buscar.mjs` verde e o Vercel `api/buscar.mjs` idêntico em comportamento.
