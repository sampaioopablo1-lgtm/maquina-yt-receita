...
- O SITE DO PEDIDO, https://jazz-conecta-imoveis.netlify.app/, É A LP `lp-oportunidades/` deste repositório (descoberto em 04/09 pelo bot do Netlify no PR #44: o site está ligado ao repo e publica `lp-oportunidades/public` + a função `netlify/functions/buscar.mjs` em `/api/buscar`, que chama a GeckoAPI; regra toda em `buscar-core.mjs`; testes `node teste-buscar.mjs`, sem rede; NÃO usa Supabase). Ele mora em OUTRA conta/time Netlify, não no `imobjazz`, então os logs da função `/api/buscar` NÃO são legíveis pelo MCP até o Pablo mover o site para o time `imobjazz` ou conectar essa conta. Registro por busca (uma linha JSON, sem dado pessoal) entrou no PR #49; até ele ir para a main, a função não registra nada.
...
a0. [EM PR #49 — não refazer; cuidar do PR] Registrar cada busca da LP: uma linha JSON por chamada em `buscar-core.mjs` (parâmetros, portais consultados, total por portal, motivo, duração; IP truncado; NUNCA nome/telefone) — log de função no painel do Netlify sem depender do Supabase. `node teste-buscar.mjs` = 113 verde. Lado do app: PR #50 faz o mesmo na Edge Function `buscar-imoveis` (evento busca_app); depois do merge, o passo 1(a) passa a ler essas linhas em `function_edge_logs`.
...
d. UX da busca: 3 cliques ou menos até o primeiro resultado; mobile first; estado vazio com sugestão de ampliar filtro.
