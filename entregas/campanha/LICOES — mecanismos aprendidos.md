...
- 16/09 08h: editar targeting de um conjunto pelo MCP (`ads_update_entity`) derruba os anúncios de formulário antigos pra WITH_ISSUES ("Terms of Service Not Accepted") mesmo com a página tendo aceitado os termos. Os anúncios criados pelo Windsor não caem. Correção: Windsor `enable_ad` em cada um; voltam em PENDING_REVIEW e ficam ACTIVE em ~1h. Toda rotina que ler anúncios deve procurar WITH_ISSUES e religar.
- 16/09: TAMANHO DE PÚBLICO dá pra ler pela API: Composio `METAADS_GET_OBJECT` com object_id = id do conjunto e fields ["delivery_estimate"] → estimate_mau_lower/upper_bound. Funciona pra conjunto já salvo (não pra targeting hipotético). Regra dentro da caixa = OU; entre caixas ("refinar") = E.
- 16/09 13h: TROCAR FORMULÁRIO de anúncio = criar anúncio novo. Windsor `create_ad` com `creative.object_story_spec {page_id, instagram_user_id, link_data {image_hash, link "http://fb.me/", message, name, description, call_to_action {type SEE_DETAILS, value {lead_gen_form_id}}}}` cria anúncio de formulário com o form escolhido, sem erro de termos; `enable_ad` pelo Windsor liga. `update_ad_creative` não troca form; MCP `ads_create_creative` não tem parâmetro de form. Aprendizado é do conjunto: trocar anúncios não reinicia.
- 16/09 13h: anúncio ARQUIVADO no Gerenciador não pode ser religado (erro 1885088) — só recriar. Se uma chamada do Windsor for cancelada no meio, conferir a lista antes de repetir: pode ter criado mesmo assim (deu duplicata).
- 16/09 14h: IMAGEM NOVA sem o upload do MCP (que ainda não está liberado pra conta: "gradually rolled out"): gerar no sandbox do Composio (PIL; fontes variáveis do Google Fonts via raw.githubusercontent), hospedar em uguu.se (POST https://uguu.se/upload files[]; serve image/jpeg direto — tmpfiles.org o Meta NÃO baixa, catbox recusa o uploader) e criar pelo Windsor `create_ad` com `object_story_spec.link_data.picture = <url>` (o Meta baixa e vira image_hash). `proxy_execute` do Composio está bloqueado pro metaads; `METAADS_UPLOAD_AD_IMAGE` exige s3key. Higgsfield: sem créditos no plano free.
- 16/09 14h: o nicho não se filtra por interesse (Meta removeu os interesses profissionais); se filtra por LISTA (CNAE) e por CRIATIVO que fala com o dono. Teste nichado entra no INTERESSE, mesma verba, sem conjunto novo.

- **Criar conjunto**: MCP `ads_create_ad_set` falha com erro de termos de lead ads (1815089). Usar Windsor `execute_action` create_adset (connector facebook, account 1695865631502778). Na `excluded_geo_locations`, cidade precisa `key` numérico + `radius` + `distance_unit`, sem `location_types`, senão a Meta recusa ao ativar (1487756 "localizações em conflito").
- **Interesses de saúde/estética** (harmonização, botox, estética facial, medicina estética) não aparecem mais na busca de targeting da Meta. Sobrou "Mídia de cosmetologia" 6811343488093 e cargos Esteticista 176835319023033 / Cosmetóloga 428147460629365.
