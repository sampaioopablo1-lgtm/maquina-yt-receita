...
- **10 eixos temáticos rotacionados** — força variação estrutural na origem
- Título quase idêntico a outro do canal → bloqueia
- Divulgação de conteúdo sintético automática no upload
- Revisão humana disponível (`exigir_revisao`), desligada por padrão na automação atual —
  o pipeline do GitHub Actions publica direto quando o token do canal está configurado
