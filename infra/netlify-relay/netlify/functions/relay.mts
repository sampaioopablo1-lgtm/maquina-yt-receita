...
// Relé MCP para o servidor oficial da Meta (https://mcp.facebook.com/ads).
// Recebe do claude.ai, injeta o token do usuário do sistema (variável de ambiente
// META_ACCESS_TOKEN do site no Netlify), repassa para a Meta e devolve a resposta.
// O segredo entra no caminho: /mcp/<MCP_SHARED_SECRET>. Sem ele, 404.
// Nunca devolve 401 ao cliente: quem tem o segredo já está autorizado.
...
export default async (req: Request) => {
  const TOKEN = Netlify.env.get("META_ACCESS_TOKEN") ?? "";
  const SEGREDO = Netlify.env.get("MCP_SHARED_SECRET") ?? "";
  const TETO = Number(Netlify.env.get("META_MAX_DAILY_BUDGET_CENTS") ?? "5000");
  const p = new URL(req.url).pathname;

  if (!SEGREDO || !p.startsWith("/mcp/" + SEGREDO)) return new Response("nao encontrado", { status: 404 });
  if (!TOKEN) return json({ jsonrpc: "2.0", id: null, error: { code: -32000, message: "META_ACCESS_TOKEN nao configurado no Netlify" } }, 500);

  if (req.method === "GET") return json({ servidor: "rele meta-ads-opc (netlify)", destino: META_MCP, teto_orcamento_centavos: TETO });
