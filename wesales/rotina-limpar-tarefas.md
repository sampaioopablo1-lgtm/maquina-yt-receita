...
Para um relatório somente leitura, sem concluir tarefas, remover tags ou
alterar qualquer outro dado, use `python tools/report_overdue_tasks.py`.
Configure `GHL_PIT` ou mantenha `.local/_ghl_pit.txt`; a saída JSON contém
somente metadados sanitizados das tarefas abertas vencidas e a busca é paginada.
O script não substitui esta rotina de limpeza.
