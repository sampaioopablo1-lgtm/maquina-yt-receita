...
| 9a | **Publicar a `Triagem da Nutrição`** (hoje em rascunho) | fecha **3 dos 5** achados do `auditoria_tags.py` de uma vez: é ela que remove `nutricao-90d` e `cad-inbound` sem ser arrancada por ninguém. Hoje nada **publicado** remove `nutricao-90d`. É um clique (§2.33.2) |
| 9b | **Dois `Remove Tag`** — `fechar-horario` no nó 4 do `Pós-agendamento v2`, e `cadencia-12x30-p2` nos três que arrancam (`Fechar Horário` nó 0, `Pós-agendamento v2` nó 4, `Pós-ligação v2` nós 54 e 127) | os 2 achados que sobram depois do 9a. `remove_from_workflow` não roda os nós de saída do alvo. A `cadencia-12x30-p2` é a mais cara: a parte 1 tem portão por tag e o lead marcado pode não reentrar (§2.31.2 e §2.33) |
