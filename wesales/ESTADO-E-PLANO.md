...
| 0 | **Uma pergunta, não uma limpeza: `Francisca` (+5512981913254) é você testando ou é gente?** Conferi os cinco contato por contato e **quatro são do próprio projeto**: `Pablo Sampaio` = o 9940, `O Próximo Cliente` = o 1407 da Stevo, `Sem Nome` = a própria agência (e-mail `agencia.proximocliente@gmail.com`), `156766977421470` = artefato sintético (800 dos EUA, nome = id de plataforma). Minha nota anterior dizia que os cinco eram teste sem marcação e que o A9 os perdia — errado nos dois pontos | se a `Francisca` é pessoa real que escreveu no WhatsApp, **é lead inbound parado `open` em NOVO LEAD sem dono desde 22/09 23:34** — o risco contrário do que eu havia descrito, e pior |
...
| 9d | **Duas linhas no `patch_condicoes_etapa.py`** (edição sua, o script escreve na conta): acrescentar `NOVO LEAD → etapa-novo-lead`, `NEGOCIAR → etapa-negociar` e `FORMALIZAR → etapa-formalizar` em `traduz()`, e trocar `ALVOS` (lista fixa de 10) por "todos os publicados menos a `Cadência 12x30` parte 1" | hoje não dói, mas `traduz()` devolve `None` para essas três etapas e o patch deixa a condição **inalterada e sem aviso**. O `Lead Esquecido em NOVO LEAD` é o candidato natural a ganhar gatilho de tag algum dia, e nesse dia o patch não vai saber traduzir a etapa dele (§2.33.5) |
| 9c | **`Canal que conectou` existe e ninguém escreve nele** — acrescentar um `update_contact_field` no `Pós-ligação v2`, nos mesmos nós que gravam `Resultado da tentativa` | a D10 do `PLANO-MULTICANAL.md` manda marcar o canal "junto com o resultado" e isso não foi montado: o id não aparece em nenhum dos 33 dumps. Sem ele, a decisão D6 (WhatsApp primeiro, ligação normal na segunda) fica sem dado — dá para dizer que trocou de canal, não qual funcionou (§2.34) |
