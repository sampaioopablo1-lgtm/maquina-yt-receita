...
| 9e | **Portar os 6 nós de portão de capacidade da `Cadência 12x30` para a `Cadência Inbound`** (0 nós hoje) | a Faxina **põe** `sdr-lotado` nos leads em CONECTAR, inclusive inbound, e a Inbound **não lê** a tag. O freio está engatado e a roda que gira não está ligada nele — e é a Inbound que carrega os 49 leads do "só inbound". O `GUIA-SDR.md` promete o teto ao SDR sem ressalva (§2.33.6) || 10 | **Decidir o `fila-wa`**: apagar a lista 8.3 e tirar os 75 nós, ou devolver fila própria ao WhatsApp | a tag é removida em 75 nós e aplicada em nenhum — a `Fila WhatsApp Hoje` nunca pode encher. Não é bug: o `PLANO-MULTICANAL.md` fez o WhatsApp virar parte do toque. O desatualizado é a lista 8.3. Recomendo apagar (seção 2.33) |
