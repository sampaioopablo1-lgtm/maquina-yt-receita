...
(async () => {
  ...
  const sub = page.getByText(/^Transcrição de chamada$/).last();
  if (await sub.count()) { await sub.click({ force: true }).catch(() => {}); await page.waitForTimeout(8000); }
  t = await page.evaluate(() => document.body.innerText || '').catch(() => '');
  await page.screenshot({ path: path.join(LOCAL, 'voz-transcricao.png') });
