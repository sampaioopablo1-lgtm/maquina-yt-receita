...
|---|---|---|---|
| L-01 | Não existe campo de **data/hora do retorno** | A lista inteligente "Retornos" não tem como ordenar nem filtrar "retorno de hoje"; a tarefa `[RETORNO]` não tem vencimento correto | Criar `Data do retorno` (DATE) |
| L-02 | `Segmento` é lista, mas as opções não foram definidas | Não consigo criar o campo sem os valores | Me passar a lista de segmentos |
...
| L-09 | Lead nativo do Meta Lead Ads vira **contato**, mas não vira **oportunidade** em `FUNIL DE VENDAS` — testado ao vivo em 19/09/2026: o contato `andre` (Facebook, `attributions` confirma `adSource: facebook`) tem 0 oportunidades (`opportunities_search-opportunity`), então nunca entra em etapa nenhuma | Toda a máquina (cadência, interceptação de sinal, tudo) é construída em cima de etapa de oportunidade — sem oportunidade automática, lead do Meta nunca entra na cadência de verdade, mesmo o dono tendo confirmado antes "caiem no pipeline, novo lead" | Conferir na tela se a integração Facebook/Meta tem um toggle de "criar oportunidade automaticamente" (Configurações da integração ou do formulário de Lead Ads); sem isso, promoção `Novo lead` → oportunidade teria que ser manual ou por workflow próprio — cruza com L-07. **Especificado em 19/09/2026:** `build-wesales.md`, seção 1.3, cobre o Meta junto com toda origem — não depende mais de conferir o toggle |
| L-09b | **Nada na especificação cria oportunidade** até 19/09/2026 — `grep "Create Opportunity"` no `build-wesales.md` dava zero até então. Todos os nós leem ou movem etapa; nenhum criava o registro | Não é só o Meta: contato que nasce do formulário, da API, de importação ou digitado na mão também fica sem oportunidade, logo sem etapa, logo fora da cadência. Confirmado por `opportunities_search-opportunity` em 19/09/2026: 0 oportunidades, e a base já tinha crescido para 40 contatos, maioria leads reais do Meta | Resolver **dentro da máquina**, não no toggle da integração, porque tem de valer para toda origem. **Especificado em 19/09/2026:** workflow "Porta de Entrada" (`build-wesales.md`, seção 1.3) — gatilho `Contact Created`, ação `Create/Update Opportunity` para `FUNIL DE VENDAS` → `NOVO LEAD`. Falta só a criação manual na tela (workflow não sai por API) e o backfill dos 40 contatos já existentes, descrito na mesma seção. Continua cruzando com L-07 (promoção `NOVO LEAD` → `CONECTAR`), que este item não resolve de propósito |
