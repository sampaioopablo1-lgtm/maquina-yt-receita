...
|---|---|---|---|
| L-01 | ~~Não existe campo de **data/hora do retorno**~~ — **fechada por inteiro em 22/09/2026** | A lista inteligente "Retornos" não tinha como ordenar nem filtrar "retorno de hoje"; a tarefa `[RETORNO]` não tinha vencimento correto | **Campo e fiação prontos:** `Data de retorno`/`Hora do retorno` existem na tela (`CONFERENCIA-CAMPOS.md`, Tabela K); o nó 4/4b do ramo `Pediu retorno` (`build-wesales.md`, seção 4) e a ordenação de dois níveis da lista `Retornos` (seção 8.4) estão especificados. Resta só a montagem manual na tela, mesma fila dos demais nós ainda não publicados |
