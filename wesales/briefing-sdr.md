...
| L-09 | Lead nativo do Meta Lead Ads vira **contato**, mas não vira **oportunidade** em `FUNIL DE VENDAS` — testado ao vivo em 19/09/2026: o contato `andre` (Facebook, `attributions` confirma `adSource: facebook`) tem 0 oportunidades (`opportunities_search-opportunity`), então nunca entra em etapa nenhuma | Toda a máquina (cadência, interceptação de sinal, tudo) é construída em cima de etapa de oportunidade — sem oportunidade automática, lead do Meta nunca entra na cadência de verdade, mesmo o dono tendo confirmado antes "caiem no pipeline, novo lead" | Conferir na tela se a integração Facebook/Meta tem um toggle de "criar oportunidade automaticamente" (Configurações da integração ou do formulário de Lead Ads); sem isso, promoção `Novo lead` → oportunidade teria que ser manual ou por workflow próprio — cruza com L-07 |
| L-09b | **Nada na especificação cria oportunidade.** `grep "Create Opportunity"` no `build-wesales.md` inteiro: zero. Todos os nós leem ou movem etapa; nenhum cria o registro | Não é só o Meta: contato que nasce do formulário, da API, de importação ou digitado na mão também fica sem oportunidade, logo sem etapa, logo fora da cadência. Confirmado por `opportunities_search-opportunity` em 19/09/2026: 0 oportunidades na subconta com 7 contatos dentro | Resolver **dentro da máquina**, não no toggle da integração, porque tem de valer para toda origem: o GHL tem a ação nativa `Create/Update Opportunity` em workflow. Um workflow de porta de entrada (gatilho `Contact Created`, ou `Tag Added` de `cad-inbound`/`cad-outbound`) cria a oportunidade em `FUNIL DE VENDAS` → `NOVO LEAD` e só então o resto do motor existe. É o pré-requisito de tudo que já está montado, e cruza com L-07 (promoção `NOVO LEAD` → `CONECTAR`) |
