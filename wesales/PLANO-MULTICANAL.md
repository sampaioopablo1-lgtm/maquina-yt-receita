...
| D10 | Campo novo **`Canal que conectou`** (Ligação WhatsApp / Ligação normal / Mensagem), marcado junto com o resultado — quem marca é o **SDR na tela** (`GUIA-SDR.md`; a leitura "escrita automática" era engano meu, corrigido em `build-wesales.md` §2.33.7). Decisão do dono em 23/09/2026 (pendência 9c): tornar **automático** onde o ramo já sabe a resposta sozinho. Patch dessa metade escrito e validado (`dac443f`, `patch_canal_conectou.py`), **ainda não aplicado**; a extensão ao `Pós-ligação v2` espera o `patch_remove_atendeu.py` (G-18) aplicar primeiro, mesmos ramos `Atendeu`. Detalhe, "Pronto quando" e acompanhamento: `build-wesales.md` §2.41, `ROADMAP-SALES-ENGAGEMENT.md` G-21. |
...
- [~] A1 (23/09 11:25 BRT, conferido pelo fio do 9940 via MCP) **A TRI-1 não saiu às 08:30, e isso é esperado:** o último outbound no 9940 é a mensagem de 03:11 UTC; as respostas "Bh"/"Jsjs" (03:14/03:19) vieram ANTES da reconstrução, e o teste do A8 (05:03–05:04) moveu o 9940 NEGOCIAR→FORMALIZAR→REUNIÃO — hoje ele tem `cad-inbound`, `limpar-tarefas`, `etapa-reuniao` e **não tem `status-nutricao`**, então a Triagem não tem quem atender. **Para fechar o A1:** pôr o 9940 de volta em nutrição (tag `status-nutricao`) e o dono mandar UMA resposta nova do celular 9940 em horário comercial; conferir a TRI-1 uma vez em ~2 min. Precisa do dono (mensagem tem que vir do aparelho dele).
...
- [~] A2 (23/09 19:40) **`Lembretes da Reunião v2` NO AR** (`7c5cbf03`, 142 nós; v1 `ac9f3f74` em rascunho). WA+e-mail, confirmação/D-3/D-1/H-3/M-10, 4 trilhas pelo formulário (Atendimento/Processo/Demanda/Geral), H-3 cita a Dor principal quando preenchida, trava "Reunião ainda de pé?" (etapa-reuniao, sem nao-perturbe) em cada ponto. GUIA-CLOSER ganhou a leitura do formulário → trilha → oferta. **Falta o teste** (precisa de reunião de teste do 9940 marcada pela tela/link): conferir (a) o link sai em `{{appointment.meeting_location}}` — depende de Meet/Zoom como local do calendário; (b) o que `appointmentCondition: skip` faz quando a reunião está a menos de 3 dias (empilha D-3+D-1 na hora ou pula?); (c) e-mail entregue — a subconta NÃO tem domínio de envio (location.domain vazio), 9940 não tem e-mail. Antes: (23/09 00:50) Investigado: nesta versão do GHL o calendário NÃO tem aba de lembretes (Calendários → Configurações só tem preferências) — lembrete de reunião é por WORKFLOW. Falta: (a) mapear o formato do nó "esperar até X h antes da reunião" (wait tipo `appointment`) pela tela num rascunho; (b) agendamento de teste para o 9940 (sem ferramenta de criar agendamento — pela tela); com ele, marcar No Show e conferir a NS-1 do A3 chegando no 9940 uma vez. Depois: workflow "Lembretes da Reunião" (PA-CONF na hora, PA-R24, PA-R3H, PA-R30, sem `{{user.first_name}}`). Revisão do Pós-agendamento com olhos de usuário: confirmações e lembretes da reunião (PA-CONF, PA-R24, PA-R3H, PA-R30 da biblioteca) agora podem sair pela Stevo (nó `sms`) — sobe comparecimento. Montar, testar com o 9940, publicar.
...
- [~] A8 (23/09 02:05) **Parte técnica feita**: `auditoria_tags.py` tinha 5 achados → **0** ("nenhuma tag com limpeza pulada sem segunda rede no ar"); `auditoria_final.py` 0 problemas. Conserto: `Mestre de saída v2` (`c616e2d7`) agora também tira `cad-outbound`, `cadencia-12x30-p2`, `fechar-horario`, `toque`, `proposta-pendente`, `negociacao-estagnada` (`tools/patch_mestre_tags.py`, backup `_antes-patch-mestre/`). Testado no 9940: tag `fechar-horario` saiu ao mover NEGOCIAR→FORMALIZAR; `negociacao-estagnada` saiu ao mover para REUNIÃO; Espelho pôs `etapa-reuniao`. De propósito: `cad-inbound` fica (marca de origem, ninguém re-aplica); o Mestre NÃO tira o lead das cadências (o fim da 12x30 põe a oportunidade em nutrição e seria cortado — as cadências já param pelo portão de tag). Os dumps de `workflows-json/` foram re-exportados (o da Triagem estava `draft` por ter sido exportado segundos antes de publicar). Pergunta 3 (só removida, nunca aplicada: `fila-wa`, `fila-linkedin`) = sobra inofensiva, fica. **Falta**: releitura com olhos de usuário, 1 workflow por rodada.
      - **Fechar Horário** (23/09 tarde, lido ao vivo, 30 nós): portões e saída certos; `nutricao-90d` no fim É o gatilho da Nutrição nova (conferido). Dois achados de usuário, **patch pendente (edição de publicado)**: (1) **MFH2 diz "Tenho horários amanhã"** e a janela é seg–sex → na sexta oferece sábado. Trocar por "Tenho horários nos próximos dias, de manhã e à tarde — qual fica melhor pra você?". (2) MFH1/MFH2 abrem com `{{contact.first_name}},` — medido nos 56 contatos: 3 com nome ruim (`156766977421470`, `o` do 1407, `sem` da agência), **todos da casa e com `cad-inbound`** → sai no A9; nenhum lead real afetado hoje, mas a Porta de Entrada grava o nome do WhatsApp como vier. Recomendo abrir sem nome ("Oi! Foi bom falar com você…") nas mensagens automáticas.
...
- [ ] **A10 (conferir 24/09 08:30–08:40)** trava nova `MI-0 · Ainda vale mandar?` na Inbound (`tools/patch_guarda_mi0.py`, 322→325 nós, publicada 23/09 19:06): o 9940 foi posto em CONECTAR às 19:07 (fora da janela) e a execução está parada na MI-0. **Esperado: MI-0 chega no 9940 UMA vez às ~08:30** e aparece `fila-quente`. Se não chegar: a trava bloqueou quem devia passar — reverter pelo backup `_antes-guarda-mi0/`. Carlos Andrade (lead real, DND ligado 23/09 19:05) **não** pode receber nada.
- [ ] **ABERTURA DA OPERAÇÃO — segunda 28/09/2026 (decisão do dono, 23/09 19:00).** Nenhum lead real sai de NOVO LEAD antes disso. Na segunda, 08:00: (1) Carlos Andrade (`7ECnj1bSeEIm5P58Ifd9`): desligar DND; (2) lote 1 = 6 mais recentes com telefone (Carlos Andrade, Gerson, Ana Ruth, Ricardo, Andreia, Andre) → CONECTAR com dono `JdvhvOTEBTvUyRi0BXU8`; (3) acompanhar MI-0 e 1ª tarefa; depois 6/dia (rampa F-14). **Lição (23/09):** mover para CONECTAR inscreve na hora (Inbound, 12x30, Speed-to-lead) e voltar de etapa NÃO desinscreve — só mover quando for para valer.
- [x] A9 (23/09 19:10) **Fila de NOVO LEAD limpa para a abertura:** 7 contatos da casa/teste → `nao-perturbe` + oportunidade `abandoned` (nada excluído): `<test lead: dummy data>`, `Teste Não Ligar`, `ZZ TESTE ESTRUTURA`, `Sem nome` (agência), `O Próximo Cliente` (1407), `156766977421470`, `ZZ Teste Porta Inbound`. Sobram 37 leads reais + Carlos. Antes:
