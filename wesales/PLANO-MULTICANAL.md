...
- [~] A5 (23/09 00:55) Dois alertas (`tools/build_estagnacao.py`): **Proposta Pendente** (Sim há 3 dias ainda em REUNIÃO) — TESTADO na cópia ZZ (`3505680c`): tag `proposta-pendente` aplicada; **Negociação Estagnada** (5 dias em NEGOCIAR → aviso + tarefa [CLOSER] Decidir) — cópia ZZ (`ba916ae8`) na espera, conferir. Depois: publicar as reais (`python build_estagnacao.py`) e desligar as cópias ZZ. Faxina ganhou: "[CLOSER] Apresentar proposta" só vale em REUNIÃO, e tarefa automática duplicada sai (fica a mais antiga) — achado no teste (2 tarefas iguais). Antes: Negociação Estagnada (W22, F-13): está especificado e não publicado — revisar, montar com portão por tag (`etapa-negociar`), testar, publicar.
