...
- [~] A7 (23/09 01:35) Parte 1 — `conectado-hoje` ganhou removedor (saída A, em workflow à parte, sem mexer no Pós-ligação v2): `tools/build_limpa_conectado.py` (tag adicionada → espera 1 dia → remove). Cópia `ZZ TESTE Limpa conectado-hoje (24 h)` (`966d75f6`, espera 2 min) publicada; tag posta no 9940 às 04:32:29 UTC — **PRÓXIMA RODADA: conferir que a tag saiu**; se saiu: `python build_limpa_conectado.py` (real), `python desligar_zz.py --aplicar`, tirar a cópia ZZ antes da real (senão as duas disparam). Parte 2 (tela, horário com RAM livre): listas. Recomendação: **Fila Telefone Hoje = tag `fila-tel` + `etapa-conectar`** (a 12x30 põe/tira `fila-tel` a cada toque — dispensa `conectado-hoje`); Retornos = resultado Pediu retorno; Fechar Horário = tag `fechar-horario`; Nutrição = `status-nutricao`.
