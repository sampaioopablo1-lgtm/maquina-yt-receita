...
- [~] A7 (23/09 01:35) Parte 1 — `conectado-hoje` ganhou removedor (saída A, em workflow à parte, sem mexer no Pós-ligação v2): `tools/build_limpa_conectado.py` (tag adicionada → espera 1 dia → remove). Cópia `ZZ TESTE Limpa conectado-hoje (24 h)` (`966d75f6`, espera 2 min) publicada; tag posta no 9940 às 04:32:29 UTC — **PRÓXIMA RODADA: conferir que a tag saiu** → ✅ **CONFERIDO na rodada da nuvem, 23/09 04:40 UTC: saiu.** `conectado-hoje` está em exatamente **2 contatos** (`teste atendeu` e `ZZ TESTE ESTRUTURA`), os dois com `dateUpdated` 01:55 — nenhum tocado às 04:32. O 9940 não está na lista, e `triagem-enviada` também devolve 0. O removedor funciona. **Limite do método, declarado:** não consegui ler o contato do 9940 direto (a busca por texto do MCP não o localiza por "9940"), então a evidência é "a tag está em 2 contatos e nenhum é ele", não uma leitura do contato. Para a cópia real, o que importa conferir é outra coisa: a ZZ espera **2 min** e a real tem de esperar **1 dia** — e tirar a ZZ antes de publicar a real, como o próprio item já diz.; se saiu: `python build_limpa_conectado.py` (real), `python desligar_zz.py --aplicar`, tirar a cópia ZZ antes da real (senão as duas disparam). Parte 2 (tela, horário com RAM livre): listas. Recomendação: **Fila Telefone Hoje = tag `fila-tel` + `etapa-conectar`** (a 12x30 põe/tira `fila-tel` a cada toque — dispensa `conectado-hoje`); Retornos = resultado Pediu retorno; Fechar Horário = tag `fechar-horario`; Nutrição = `status-nutricao`.
