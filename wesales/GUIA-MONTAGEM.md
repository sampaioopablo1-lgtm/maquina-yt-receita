...
| **`Mestre de saída`, nó 1 — dá para fazer HOJE, e é urgente** | Acrescentar ao portão um segundo ramo que **encerra** quando `Pipeline stage` é `[FUNIL DE VENDAS] - NOVO LEAD` (`IMPLEMENTACAO-WORKFLOWS.md`, W3) | **Confirmado por API em 21/09/2026:** a Porta de Entrada cria a oportunidade em `NOVO LEAD` e 4 segundos depois o Mestre de saída grava a nota "Saída de cadência · status: open" e aplica `limpar-tarefas` — em **todo lead novo** (os 10 criados desde 19/09 nasceram com a tag). O portão só protege `CONECTAR`+`open`; a criação em `NOVO LEAD` cai no `None` e roda a limpeza inteira |
...
| `Pós-agendamento`, nó 4 (nota de qualificação) | Refazer como Math em série (`IMPLEMENTACAO-WORKFLOWS.md`, W5) ou corrigir o nó de código para **escrever** em `nota_de_qualificao` — **depois** de o dono decidir o G-04 | Os 3 leads que passaram (`Daniel`, `Genilson`, `Teste Atendeu`) têm `Prioridade` e `Data agendado` gravados e `Nota de qualificação` vazia; a nota "Checklist de Autoauditoria" que a IA deixou no contato diz que o nó foi montado como "custom code" |
