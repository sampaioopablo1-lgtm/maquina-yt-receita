...
| ~~`Negociação Estagnada` — workflow novo, dá para fazer quando alguém abrir a tela~~ **JÁ PUBLICADO — G-23, 23/09/2026** | ~~Montar o workflow inteiro...~~ — o dono já montou e publicou **dois** workflows pelo próprio caminho (`tools/build_estagnacao.py`, `PLANO-MULTICANAL.md` A5): `Negociação Estagnada` (`53334baa`, 5 dias, gatilho `Opportunity Stage Changed → NEGOCIAR` — não o desenho de 3 dias que esta linha citava) e `Proposta Pendente` (`14fdf9fa`, cobre o buraco que nenhum documento tinha especificado ainda: closer marca `Sim` e nunca move para `NEGOCIAR`). As duas tags (`negociacao-estagnada`, `proposta-pendente`) já existem. Faltam só as Smart Lists `Saúde — Negociação Estagnada` (8.28) e `Saúde — Proposta Pendente` (8.29, nova) na tela | Fechava a única transição do funil (`NOVO LEAD` → `FORMALIZAR`) que ficou sem monitor — e um segundo buraco (proposta nunca apresentada) que só apareceu ao comparar `GUIA-CLOSER.md` com o publicado. Detalhe completo: `build-wesales.md` §2.28, `IMPLEMENTACAO-WORKFLOWS.md` W22, `ROADMAP-SALES-ENGAGEMENT.md` G-23 |
| ~~`Mestre de saída`, nó 4 — dá para fazer HOJE, mas depende do workflow novo existir para valer algo~~ **JÁ FEITO, mas pelo `Mestre de saída v2` (`tools/patch_mestre_tags.py`), não por esta linha** | ~~Somar `negociacao-estagnada`...~~ — o v2 já soma `negociacao-estagnada` **e** `proposta-pendente` (e mais quatro tags fora do escopo do F-13: `cad-outbound`, `cadencia-12x30-p2`, `fechar-horario`, `toque`) | Mesmo achado de 22/09 (F-13), fechado pelo dono direto na versão publicada antes de esta linha ser riscada |
