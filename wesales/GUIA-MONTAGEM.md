...
| **`Mestre de saída`, nó 2 — dá para fazer HOJE, substitui a linha abaixo** | Apagar os `Remove from Workflow: Cadência 12x30`/`Cadência Inbound`/`Reengajamento 90 dias`/`Qualificação por IA no WhatsApp` (nós 2/2b/2c/3) e pôr um único `Remove Workflows` com a opção **`All Except Current Workflow`** no lugar (`build-wesales.md`, seção 3, nó 2) | F-05, achado de 21/09/2026: a lista manual já precisou de dois nomes a mais em 19/09/2026 e nunca cobriu as Interceptações de Sinal — a ação nativa tira o contato de toda régua ativa, existente ou futura, sem precisar nomear nenhuma, e **não depende de nenhum workflow novo existir antes**: dá para fazer hoje, ao contrário da linha que ela substitui |
