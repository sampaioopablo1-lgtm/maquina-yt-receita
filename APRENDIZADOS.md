...
Última sincronização: **2026-08-13** · 230 regras ativas, 63 críticas.
Nota: 1 registro (id 194, categoria processo) segue `status='candidato'` — nem ativo nem
invalidado, decisão pendente do Pablo — não listado abaixo por não caber no binário deste
arquivo; consulte `select * from aprendizados where status='candidato'` no Supabase.
...
> **view**: v_maquina_fila · **achado_por**: Supabase Advisor security (security_definer_view, ERROR) · **corrigido_em**: 2026-08-05 · **migration_fix**: fila_security_invoker · **gap_relacionado**: v_maquina_pendencias tambem nunca tinha sido versionada em supabase/schema.sql, apesar de existir em producao desde o inicio · **reloptions_antes**:  · **reloptions_depois**: ["security_invoker=on"] · **grants_anon_confirmados**: ["SELECT", "INSERT", "UPDATE", "DELETE"] · **migrations_que_derrubaram**: ["fila_prioriza_canal_ativo", "fila_conta_pacote_nao_linha", "fila_ignora_cancelado"]
...
`aplicado_em:` src/maquina/stages/youtube.py — ESCOPOS ganhou yt-analytics.readonly em 13/08 (branch claude/happy-curie-1dfi43); so vale para consentimento NOVO, tokens existentes precisam reautorizar via scripts/links_reauth.py
...
### O hash de trilha manda a faixa ERRADA para 10 dos 13 canais — e erra o audio, nao so o credito
Toda spec declara `trilha`, e ela vence o hash. O hash so existe como ultimo recurso. Medido baixando o bucket direto pelo sandbox em 14/08/2026: das quatro faixas da lista, TRES existem e o Cipher2 devolve 404 NoSuchKey. Com as tres reais, o sorteio por hash do slug coincide com canais.trilha em apenas 3 dos 13 canais. E nao e so o credito CC-BY que sai errado: `aplicar_trilha` chama a MESMA funcao, entao o audio mixado tambem e o da faixa errada.

> **bucket**: {"Cipher2": "HTTP 404, NoSuchKey, 88 bytes de JSON de erro", "Inspired": "HTTP 200, 9.156.393 bytes, 286,1 s", "Wholesome": "HTTP 200, 14.556.390 bytes, 363,8 s", "Deliberate_Thought": "HTTP 200, 7.224.606 bytes, 177,5 s"} · **metodo**: GET direto em storage/v1/object/videos-maquina/trilhas/<faixa>.mp3 pelo sandbox, com a ANON key · **exemplos**: ["resep-naik-level: hash da Inspired, registro diz Deliberate_Thought", "next-level-money: hash da Wholesome, registro diz Deliberate_Thought", "setiap-level: hash da Deliberate_Thought, registro diz Wholesome"] · **medido_em**: 2026-08-14 · **retificacao**: ontem eu levantei a hipotese de que as faixas do bucket poderiam ser placeholder, porque as quatro copias no container da sessao tem o mesmo md5. FALSO: as tres do bucket sao faixas reais, de 177 a 364 segundos e md5 distintos. O placeholder era so do container. · **cipher2_e_fantasma**: nenhum dos treze canais esta registrado em Cipher2. Ele estava na lista de download do frota.yml e em TRILHAS_VALIDAS sem existir e sem ninguem usar — e um fantasma na lista fazia o hash dividir por quatro quando so tres chegam. · **divergencia_hash_vs_registro**: 10 de 13 canais. So game-money-lab, seviye-seviye e sx-educacao coincidem por acaso.

`aplicado_em:` as 25 specs declaram trilha; fabrica.py:aplicar_trilha e trilha_do_canal repassam registrada; etapas.py passa sp[trilha]; copy_md.py:TRILHAS_VALIDAS sem Cipher2; frota.yml baixa tres

### 33 dos 41 videos no ar nao tem credito CC-BY, e credito e trilha andam soltos um do outro
Video sem musica e saida ERRADA, nao saida menor: `aplicar_trilha` levanta em vez de retornar calado. O `return` silencioso era invisivel duas vezes — o log escrevia "etapa 6 ok" e a marca de pronto dizia literalmente "longo montado e com trilha". Agora a marca grava QUAL faixa entrou.

> **conclusao**: credito e audio estao decorrelacionados NOS DOIS SENTIDOS. Nao e um desvio sistematico, sao dois caminhos independentes: escrever_copy e aplicar_trilha chamavam trilha_do_canal separadamente, e o segundo desistia calado. · **medido_em**: 2026-08-14 · **credito_nas_descricoes**: {"lidos": 41, "batem_o_registro": 7, "SEM_credito_nenhum": 33, "creditam_outra_faixa": 1} · **o_que_NAO_esta_provado**: se o arquivo do bucket e o MESMO que foi publicado no YouTube. A medicao decisiva seria o audio do video publicado, e baixar do YouTube esta bloqueado deste ambiente. O numero de 33 sem credito, esse sim, e leitura direta da descricao publicada. · **correcao_anterior_do_mesmo_dia**: o hash escolhia a faixa errada para 10 dos 13 canais (aprendizado 260); as 25 specs passaram a declarar `trilha` e ela vence. Com o campo declarado, faixa registrada ausente ja levanta FileNotFoundError. · **piso_de_ruido_em_4_videos_do_bucket**: {"metodo": "RMS em janelas de 0,5 s nos primeiros 90 s, percentil 5. Trilha a -28 dB poe o piso perto de -30 dB; so narracao fica perto de -48 dB.", "agla-level-003": "p05 -48,4 dB -> sem trilha, sem credito (coerente)", "setiap-level-006": "p05 -48,5 dB -> SEM trilha, e a descricao CREDITA Inspired", "kolejny-poziom-007": "p05 -48,4 dB -> sem trilha, sem credito (coerente)", "labtreinamento-001": "p05 -29,6 dB -> TEM trilha, e a descricao NAO credita"}

`aplicado_em:` fabrica/fabrica.py:aplicar_trilha levanta RuntimeError; fabrica/etapas.py grava a faixa na marca de pronto. FALTA: conferir o audio dos videos JA publicados, que exige baixar do YouTube.
...
> **fonte**: copy.md + legendas.srt · **longo**: MjI4ZGJAhIo · **short**: 7vqZHEzRP2A · **pacote**: kolejny-poziom-007 · **parado_desde**: 2026-08-11 21:58 · **roteiro_antes**:  · **cenas_reconstruidas**: 76
